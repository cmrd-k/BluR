...
print("main")
def blurVideo(videoPath):
    print("blur running")
    # defines source for video, 0 for webcam, replace 0 with filepath to use saved videos
    video = cv2.VideoCapture(videoPath)

    height = int(video.get(4)) #sets video height
    width = int(video.get(3)) #sets video width
    frameCount = int(video.get(7)) #total frame count
    framesProcessed = 0 #tracks processed frames
    ...
    while(video.isOpened()):
        emptyMask = np.zeros((height, width, 1), dtype=np.uint8)  # creates height*width black(0)->white(255) channel
        ...
        if ret:
            ...
            if cv2.waitKey(1) & 0xFF == ord("q"):
                break
            framesProcessed += 1
            fProcessedPercent = framesProcessed/frameCount * 100
            print(f'framesprocessed: {framesProcessed}/{frameCount} == {fProcessedPercent}%')
            gui.trackProgress(fProcessedPercent)
        else:
            break
